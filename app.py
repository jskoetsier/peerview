...
def inject_version():
    return {"app_version": get_version(), "version_info": get_version_info()}
...
class PeeringDashboard:
            ...
            return {
                ...
                "ixps": {
                    "amsix": {
                        ...
                        "ipv6_range": [
                            "2001:7f8:1:0:0:0:0:0",
                            "2001:7f8:1:0:ffff:ffff:ffff:ffff",
                        ],
                    },
                    "frys-ix": {
                        "pretty_name": "Frys-IX",
                        ...
                        "ipv6_range": [
                            "2001:7f8:10f:0:0:0:0:0",
                            "2001:7f8:10f:0:ffff:ffff:ffff:ffff",
                        ],
                    },
                    "speedix": {
                        "pretty_name": "SpeedIX",
                        ...
                        "ipv6_range": [
                            "2001:7f8:b7:0:0:0:0:0",
                            "2001:7f8:b7:0:ffff:ffff:ffff:ffff",
                        ],
                    },
                    "nlix": {
                        "pretty_name": "NL-IX",
                        ...
                        "ipv6_range": [
                            "2001:7f8:13:0:0:0:0:0",
                            "2001:7f8:13:0:ffff:ffff:ffff:ffff",
                        ],
                    },
                    "loc-ix": {
                        "pretty_name": "Loc-IX",
                        ...
                        "ipv6_range": [
                            "2a0c:b641:700:0:0:0:0:0",
                            "2a0c:b641:700:0:ffff:ffff:ffff:ffff",
                        ],
                    },
                    "interix": {
                        "pretty_name": "InterIX",
                        ...
                        "ipv6_range": [
                            "2001:7f8:134:0:0:0:0:0",
                            "2001:7f8:134:0:ffff:ffff:ffff:ffff",
                        ],
                    },
                    "lsix": {
                        ...
                        "ipv6_range": [
                            "2001:7f8:8f:0:0:0:0:0",
                            "2001:7f8:8f:0:ffff:ffff:ffff:ffff",
                        ],
                    },
                    "fogixp": {
                        "pretty_name": "FogIXP",
                        ...
                        "ipv6_range": [
                            "2001:7f8:ca:1:0:0:0:0",
                            "2001:7f8:ca:1:ffff:ffff:ffff:ffff",
                        ],
                    },
                },
                "warning_thresholds": {
    ...
    async def fetch_router_sessions(
        self, session: aiohttp.ClientSession, router: str, port: int
    ) -> Dict:
    ...
    async def fetch_session_definition(self, session: aiohttp.ClientSession) -> Dict:
        ...
        try:
            ...
            if not url.startswith("https://"):
                logger.error(
                    f"Refusing to fetch session definitions from non-HTTPS URL: {url}"
                )
                return {}
            timeout = aiohttp.ClientTimeout(total=30)
            async with session.get(url, timeout=timeout) as response:
    ...
    async def collect_all_sessions(self) -> Dict[str, PeerStatus]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            for router in self.config["routers"]:
                router_tasks.append(
                    self.fetch_router_sessions(session, router, 29184)
                )  # IPv4
                router_tasks.append(
                    self.fetch_router_sessions(session, router, 29186)
                )  # IPv6
            ...
            for asn, peer_data in defined_sessions.items():
                ...
                for afi, afi_sessions in [
                    ("ipv4", ipv4_sessions),
                    ("ipv6", ipv6_sessions),
                ]:
                    for router, sessions in afi_sessions.items():
                        for session_name, session_data in sessions.items():
                            if str(session_data.get("neighbor_as")) == asn.replace(
                                "AS", ""
                            ):
                                # Determine IXP
                                ...
                                if ix_name:
                                    ...
                                    peer_status.sessions[afi][ix_name].append(
                                        bgp_session
                                    )
    ...
    def get_session_status_class(self, session: BGPSession) -> str:
        ...
        try:
            since_str = session.since
            # Handle ISO format with Z suffix (e.g., "2026-03-12T07:24:33Z")
            if "Z" in since_str:
                since_time = datetime.fromisoformat(since_str.replace("Z", "+00:00"))
            # Handle ISO format with timezone offset (e.g., "2026-03-12T07:24:33+00:00")
            elif "T" in since_str and ("+" in since_str or since_str.endswith("Z")):
                since_time = datetime.fromisoformat(since_str)
            # Handle plain datetime format from router (e.g., "2026-03-12 07:24:33")
            else:
                since_time = datetime.strptime(since_str, "%Y-%m-%d %H:%M:%S").replace(
                    tzinfo=timezone.utc
                )
    ...
    def filter_peers(
        self, peers: Dict[str, PeerStatus], filters: Dict
    ) -> Dict[str, PeerStatus]:
                ...
                for afi in ["ipv4", "ipv6"]:
                    ...
                    if filter_value and filter_value != "any":
                        ...
                        if filter_value == "established":
                            ...
                            if not (
                                ix_sessions
                                and any(s.state != "Established" for s in ix_sessions)
                            ):
                                should_include = False
                                break
...
async def index():
    ...
    filters = {
        ...
        "ixes": request.args.get("ixes", "").split(",")
        if request.args.get("ixes")
        else None,
    }
